...
func (c *OpcClient) Subscribe() (chan opc.Message, chan struct{}) {
...
func (c *OpcClient) removeWhenDone(listener chan opc.Message, done chan struct{}) {
	<-done
...
func main() {
	...
	if conn, err := net.Dial("tcp", *addr); err != nil {
		...
		if *yaml != "" {
			opcConn.SendSysEx(0, opc.CmdSetEffectsYaml, readFile(*yaml))
		} else if *json != "" {
			opcConn.SendSysEx(0, opc.CmdSetEffectsJson, readFile(*json))
		}
		...
		go func() {
			...
			for scanner.Scan() {
				...
				case "o", "open":
					...
					buffer = nil
				case "e", "effects":
					go opcConn.SendSysEx(0, opc.CmdSetEffectsStreamFps, []byte{0})
					msg := opcClient.WaitForMsg(func(msg opc.Message) bool {
